...
const btcUri = process.env.BTCEXP_SILVERD_URI ? url.parse(process.env.BTCEXP_SILVERD_URI, true) : { query: { } };
...
function loadFreshRpcCredentials() {
	let username = btcAuth[0] || process.env.BTCEXP_RPC_USERNAME;
	let password = btcAuth[1] || process.env.BTCEXP_RPC_PASSWORD;

	let authCookieFilepath = btcUri.query.cookie || process.env.BTCEXP_SILVERD_COOKIE || path.join(os.homedir(), '.silverbitcoin', '.cookie');
	...
	return {
		host: btcUri.hostname || process.env.BTCEXP_RPC_HOST || "127.0.0.1",
		port: btcUri.port || process.env.BTCEXP_RPC_PORT || 8332,
		...
		timeout: parseInt(btcUri.query.timeout || process.env.BTCEXP_RPC_TIMEOUT || 5000),
	};
}
