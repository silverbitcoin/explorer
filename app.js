...
const debugDefaultCategories = "slvrexp:app,slvrexp:error,slvrexp:errorVerbose";
debug.enable(debugDefaultCategories);


const debugLog = debug("slvrexp:app");
const debugErrorLog = debug("slvrexp:error");
const debugPerfLog = debug("slvrexp:actionPerformace");
const debugAccessLog = debug("slvrexp:access");

const configPaths = [
	path.join(os.homedir(), ".config", "slvr-rpc-explorer.env"),
	path.join("/etc", "slvr-rpc-explorer", ".env"),
...
async function onRpcConnectionVerified(getnetworkinfo, getblockchaininfo) {
	...
	var bitcoinCoreVersionRegex = /^.*\/SilverBitcoinCore\:(.*)\/.*$/;
	...
	if (match) {
		...
		debugErrorLog(`Unable to parse node version string: ${getnetworkinfo.subversion} - RPC versioning will likely be unreliable. Is your node a version of SilverBitcoin Core?`);
	}
